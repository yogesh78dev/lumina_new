...
const ClosedLeadsPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<ActiveTab>('won');
  ...
  const closedLeads = useMemo(() => {
    const targetStatus = activeTab === 'won' ? 'Won' : 'Lost';
    return leads
      .filter(lead => lead.leadStatus === targetStatus)
      .filter(lead => lead.name.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [leads, activeTab, searchTerm]);
  
  // FIX: Implement sorting for the table data.
  const { items: sortedLeads, requestSort, sortConfig } = useSorting<Lead>(closedLeads);
            ...
            <div className="flex items-center space-x-2">
                ...
                />
                 <div className="flex items-center p-1 bg-gray-100 rounded-lg">
                    <button onClick={() => setActiveTab('won')} className={tabClass('won')}>
                        <i className="fa-solid fa-trophy mr-2"></i>Won
                    </button>
                    <button onClick={() => setActiveTab('lost')} className={tabClass('lost')}>
                        <i className="fa-solid fa-heart-crack mr-2"></i>Lost
                    </button>
                </div>
